...
export default function DoctorAdvice() {
    ...
    return (
        <section className="py-20 sm:py-22 space-y-20 sm:space-y-24 px-4 sm:px-32 rounded-4xl  relative overflow-hidden">
            {/* Background Accent */}
            <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-gradient-to-bl from-blue-50/10 via-black/20 to-transparent rounded-full blur-3xl -z-10 -mr-64 -mt-64"></div>
            ...
            <div className="flex flex-col lg:flex-row items-center justify-between gap-12 sm:gap-16 mx-1 sm:mx-5 sm:px-6 md:px-10">
                <div className="space-y-6 sm:space-y-8 max-w-2xl text-center lg:text-left">
                    ...
                    <h2 className="text-4xl font-black text-gray-200 leading-[1.1] tracking-tight">
                        {language === 'hi' ? 'सलाह' : 'Advice'} <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">{t.aboutAdvice.thoughts}</span>
                    </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 sm:gap-12 mx-1 sm:mx-5">
                {tips.map((tip, index) => (
                    <div
                        key={index}
                        className="group bg-white p-10 rounded-[3rem] border border-slate-200 hover:border-blue-200 transition-all duration-500 shadow-xl hover:shadow-2xl hover:-translate-y-1 relative overflow-hidden"
                    >
                        <div className="absolute top-0 right-0 w-24 h-24 bg-gray-50 rounded-bl-full group-hover:bg-blue-50 transition-colors"></div>
                        <div className="mb-10 w-16 h-16 bg-gray-50 rounded-3xl flex items-center justify-center group-hover:bg-blue-300 group-hover:text-white transition-all duration-500 shadow-inner overflow-hidden relative z-10">
                            {tip.icon}
                        </div>
                        <div className="space-y-4 relative z-10">
                            <h3 className="text-2xl font-black text-slate-900 group-hover:text-blue-700 transition-colors">{tip.title}</h3>
                            <p className="text-slate-600 text-base sm:text-md leading-relaxed font-medium">
                                {tip.content}
                            </p>
                        </div>
                    </div>
            ...
            <div className="bg-gradient-to-br from-gray-900 to-slate-900 rounded-[3rem] p-12 sm:p-24 mx-1 sm:mx-5 text-white flex flex-col items-center text-center gap-10 sm:gap-16 shadow-[0_0px_50px_-20px_rgba(0,0,0,0.5)] relative overflow-hidden group">
                {/* Decorative Elements */}
