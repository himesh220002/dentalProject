...
export default function QuickScheduler({ isOpen, onClose, onSuccess, initialDate, initialSearch, initialName, initialEmail, messageId, appointmentId, inquiryMessage, initialPatientId, skipWhatsApp }: QuickSchedulerProps) {
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-[#fcfcfc] flex flex-col overflow-hidden w-full max-w-full">
            {/* Header */}
            <div className="h-[64px] bg-white border-b border-black/5 flex items-center justify-between px-4 sm:px-6 lg:px-8 shrink-0 w-full max-w-full min-w-0 overflow-hidden">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                    <button onClick={onClose} className="w-9 h-9 rounded-full bg-[#f5f5f3] border border-black/5 grid place-items-center text-neutral-700 hover:bg-black hover:text-white hover:border-black transition shrink-0"><FaTimes size={14} /></button>
                    <div className="min-w-0">
                        <h1 className="text-[15px] font-semibold tracking-[-0.01em] text-[#0a0a0b] flex items-center gap-2 min-w-0 truncate"><FaCalendarAlt size={13} className="text-neutral-500 shrink-0" /> <span className="truncate">Quick Scheduler</span></h1>
                        <p className="text-[11px] tracking-[0.08em] uppercase font-medium text-neutral-500 hidden sm:block truncate">{appointmentId ? 'Reschedule Appointment' : 'New Appointment — Schedule with clarity'}</p>
                    </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                    <span className="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-[#f5f5f3] border border-black/5 text-[11px] font-medium text-neutral-600 max-w-[150px] truncate">{formData.date} {formData.time && `· ${formData.time}`}</span>
                    <span className="hidden lg:inline-flex text-[11px] font-medium px-3 py-1.5 rounded-full bg-emerald-50 border border-emerald-100 text-emerald-700 shrink-0">₹{totalAmount.toLocaleString()} estimated</span>
                </div>
            </div>

            {statusMessage && (
                <div className="mx-4 sm:mx-6 lg:mx-8 mt-4 bg-white border border-black/5 rounded-[16px] p-4 flex items-center gap-3 shadow-sm max-w-[calc(100%-2rem)] sm:max-w-[calc(100%-3rem)] overflow-hidden min-w-0">
                    <span className={`w-8 h-8 rounded-full grid place-items-center shrink-0 ${statusMessage.type === 'success' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-rose-50 text-rose-600 border border-rose-100'}`}><FaCheck size={12} /></span>
                    <span className="text-[13px] font-medium text-[#0a0a0b] min-w-0 break-words flex-1">{statusMessage.text}</span>
                </div>
            )}

            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto overflow-x-hidden w-full">
                <div className="max-w-[1280px] mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 pb-24 grid lg:grid-cols-3 gap-6 min-w-0">
                    {/* Left — main form */}
                    <div className="lg:col-span-2 space-y-5 min-w-0 w-full max-w-full overflow-hidden">
                        {/* Patient */}
                        <div className="bg-white rounded-[20px] border border-black/5 p-5 sm:p-6 w-full max-w-full min-w-0 overflow-hidden">
                            ...
                            {!isNewPatientMode ? (
                                <div className="space-y-3 min-w-0 w-full max-w-full overflow-hidden">
                                    <div className="relative min-w-0 w-full max-w-full">
                                        ...
                                        <input type="text" placeholder="Search by name or phone" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full max-w-full min-w-0 h-[44px] pl-9 pr-4 rounded-full bg-[#fcfcfc] border border-black/5 focus:bg-white focus:border-black/10 outline-none text-[13px] font-medium placeholder:text-neutral-400" />
                                    </div>
                                    <div className="relative min-w-0 w-full max-w-full">
                                        <select required={!isNewPatientMode} value={formData.patientId} onChange={(e) => setFormData({ ...formData, patientId: e.target.value })} className="w-full max-w-full min-w-0 h-[44px] px-4 rounded-full bg-[#fcfcfc] border border-black/5 focus:bg-white focus:border-black/10 outline-none text-[13px] font-medium appearance-none">
                                    ...
                                    {selectedPatient && (
                                        <div className="flex items-center gap-2 p-3 rounded-2xl bg-[#f5f5f3] border border-black/5 text-[13px] min-w-0 w-full max-w-full overflow-hidden">
                                            <span className="w-7 h-7 rounded-full bg-white border border-black/5 grid place-items-center text-neutral-700 shrink-0"><FaUser size={11} /></span>
                                            <span className="font-medium tracking-[-0.01em] text-[#0a0a0b] truncate min-w-0">{selectedPatient.name}</span>
                                            <span className="text-neutral-500 truncate min-w-0">· {selectedPatient.contact}</span>
                                            <span className="ml-auto text-[11px] px-2 py-1 rounded-full bg-white border border-black/5 font-medium shrink-0">{selectedPatient._id.slice(-6).toUpperCase()}</span>
                                        </div>
                                    )}
                                </div>
                            ) : (
                                <div className="grid sm:grid-cols-3 gap-3 min-w-0 w-full max-w-full">
                                    <input required placeholder="Full name" value={newPatientData.name} onChange={(e) => setNewPatientData({ ...newPatientData, name: e.target.value })} className="h-[44px] px-4 rounded-full bg-[#fcfcfc] border border-black/5 focus:bg-white focus:border-black/10 outline-none text-[13px] font-medium w-full max-w-full min-w-0" />
                                    <input required placeholder="Phone (10 digits)" value={newPatientData.contact} onChange={(e) => setNewPatientData({ ...newPatientData, contact: e.target.value.replace(/\D/g, '').slice(0, 10) })} className="h-[44px] px-4 rounded-full bg-[#fcfcfc] border border-black/5 focus:bg-white focus:border-black/10 outline-none text-[13px] font-medium w-full max-w-full min-w-0" />
                                    <input placeholder="Age (optional)" type="number" value={newPatientData.age} onChange={(e) => setNewPatientData({ ...newPatientData, age: e.target.value })} className="h-[44px] px-4 rounded-full bg-[#fcfcfc] border border-black/5 focus:bg-white focus:border-black/10 outline-none text-[13px] font-medium w-full max-w-full min-w-0" />
                                </div>
                            )}
                        </div>
                        ...
                        <div className="bg-white rounded-[20px] border border-black/5 p-5 sm:p-6 w-full max-w-full min-w-0 overflow-hidden">
                            ...
                            <div className="mt-3 grid sm:grid-cols-2 gap-3 min-w-0 w-full max-w-full">
                                ...
                                <div className="relative min-w-0 w-full max-w-full">
                                    ...
                                    <input type="time" required value={formData.time} onChange={(e) => setFormData({ ...formData, time: e.target.value })} className="w-full max-w-full min-w-0 h-[44px] pl-9 pr-4 rounded-full bg-[#fcfcfc] border border-black/5 focus:bg-white focus:border-black/10 outline-none text-[13px] font-medium" />
                                </div>
                            </div>
                        ...
                        <div className="bg-white rounded-[20px] border border-black/5 p-5 sm:p-6 w-full max-w-full min-w-0 overflow-hidden">
                            ...
                            <div className="mt-3 space-y-2 min-w-0 w-full max-w-full">
                                {formData.selectedTreatments.map((treatment, index) => (
                                    <div key={index} className="flex gap-2 items-center min-w-0 w-full max-w-full">
                                        ...
                                        <select required aria-label="Select Treatment" value={treatment.name} onChange={(e) => handleTreatmentChange(index, e.target.value)} className="flex-1 min-w-0 w-full max-w-full h-[44px] px-4 rounded-full bg-[#fcfcfc] border border-black/5 focus:bg-white focus:border-black/10 outline-none text-[13px] font-medium">
                                ...
                                <button type="button" onClick={addTreatmentRow} className="w-full max-w-full min-w-0 h-10 rounded-full border border-dashed border-black/10 text-[12px] font-medium text-neutral-600 hover:bg-[#fcfcfc] hover:border-black/15 flex items-center justify-center gap-2"><FaPlusCircle size={11} /> Add Another Treatment</button>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-[20px] border border-black/5 p-5 sm:p-6 w-full max-w-full min-w-0 overflow-hidden">
                            <div className="flex items-center justify-between gap-2 min-w-0">
                                <h3 className="text-[11px] tracking-[0.12em] uppercase font-medium text-neutral-500 flex items-center gap-2 min-w-0"><FaMoneyBillWave size={11} className="shrink-0" /> <span className="truncate">Billing</span></h3>
                                <span className="text-[16px] font-semibold tracking-[-0.02em] text-[#0a0a0b] shrink-0">₹{totalAmount.toLocaleString()}</span>
                            </div>
                            <div className="mt-3 grid grid-cols-2 gap-3 text-[12px] min-w-0 w-full max-w-full">
                                <div className="rounded-2xl bg-[#fcfcfc] border border-black/5 p-3 min-w-0 w-full max-w-full overflow-hidden"><div className="text-neutral-500">Base</div><div className="font-semibold text-[#0a0a0b] mt-1 truncate">₹{basePriceTotal.toLocaleString()}</div></div>
                                <div className="rounded-2xl bg-[#fcfcfc] border border-black/5 p-3 min-w-0 w-full max-w-full overflow-hidden">
                                    ...
                                    }} className="mt-1 w-full max-w-full min-w-0 h-8 px-3 rounded-full bg-white border border-black/5 text-[11px] font-medium outline-none">
                            ...
                            {(formData.selectedTreatments.length > 0 || formData.additionalItems.length > 0) && (
                                <div className="mt-3 pt-3 border-t border-black/5 space-y-1 min-w-0 w-full max-w-full overflow-hidden">
                                    {formData.selectedTreatments.filter(t => t.name).map((t, i) => <div key={i} className="flex justify-between text-[12px] gap-2 min-w-0"><span className="text-neutral-600 truncate min-w-0 flex-1">{t.name}</span><span className="font-medium text-[#0a0a0b] shrink-0">₹{t.price}</span></div>)}
                                    {formData.additionalItems.map((it, i) => (
                                        <div key={i} className="flex justify-between items-center text-[12px] gap-2 min-w-0">
                                            <span className="text-neutral-600 flex items-center gap-2 min-w-0 flex-1 truncate">{it.name} <button type="button" onClick={() => setFormData(prev => ({ ...prev, additionalItems: prev.additionalItems.filter((_, idx) => idx !== i) }))} className="w-5 h-5 rounded-full bg-white border border-black/5 grid place-items-center text-rose-500 hover:bg-rose-50 shrink-0"><FaTimes size={8} /></button></span>
                                            <span className="font-medium text-[#0a0a0b] shrink-0">₹{it.price}</span>
                                        </div>
                                    ))}
                                </div>
                            )}
                            <div className="mt-4 min-w-0 w-full max-w-full">
                                ...
                                <textarea value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} placeholder="Complaint, findings, procedure..." rows={3} className="mt-2 w-full max-w-full min-w-0 p-3 rounded-2xl bg-[#fcfcfc] border border-black/5 focus:bg-white focus:border-black/10 outline-none text-[13px] leading-6 resize-none" />
                            </div>
                        </div>
                    </div>

                    {/* Right — summary */}
                    <div className="space-y-4 lg:sticky lg:top-6 self-start w-full max-w-full min-w-0 overflow-hidden">
                        <div className="bg-white rounded-[20px] border border-black/5 p-5 w-full max-w-full min-w-0 overflow-hidden">
                            <h3 className="text-[11px] tracking-[0.12em] uppercase font-medium text-neutral-500">Summary</h3>
                            <div className="mt-3 space-y-2 text-[13px] min-w-0 w-full max-w-full">
                                <div className="flex justify-between gap-2 min-w-0"><span className="text-neutral-500 shrink-0">Patient</span><span className="font-medium text-[#0a0a0b] truncate min-w-0 flex-1 text-right">{isNewPatientMode ? (newPatientData.name || 'New patient') : (selectedPatient?.name || '—')}</span></div>
                                <div className="flex justify-between gap-2 min-w-0"><span className="text-neutral-500 shrink-0">Date</span><span className="font-medium text-[#0a0a0b] truncate min-w-0 flex-1 text-right">{formData.date || '—'}</span></div>
                                <div className="flex justify-between gap-2 min-w-0"><span className="text-neutral-500 shrink-0">Time</span><span className="font-medium text-[#0a0a0b] truncate min-w-0 flex-1 text-right">{formData.time || '—'}</span></div>
                                <div className="flex justify-between gap-2 min-w-0"><span className="text-neutral-500 shrink-0">Treatments</span><span className="font-medium text-[#0a0a0b] text-right max-w-[120px] sm:max-w-[160px] truncate min-w-0 flex-1">{formData.selectedTreatments.filter(t => t.name).map(t => t.name).join(', ') || '—'}</span></div>
                                <div className="pt-2 mt-2 border-t border-black/5 flex justify-between text-[13px] font-semibold tracking-[-0.01em] gap-2"><span>Total</span><span>₹{totalAmount.toLocaleString()}</span></div>
                            </div>
                        </div>
                        <div className="bg-[#0a0a0b] rounded-[20px] p-5 text-white w-full max-w-full min-w-0 overflow-hidden">
                            <div className="text-[11px] tracking-[0.12em] uppercase font-medium text-white/60">Confirm</div>
                            <p className="text-[12px] leading-5 text-white/60 mt-1 break-words">WhatsApp confirmation will be sent to patient and staff after scheduling.</p>
                            <div className="mt-4 grid grid-cols-2 gap-2 min-w-0 w-full max-w-full">
                                <button type="button" onClick={onClose} className="h-10 rounded-full bg-white/10 border border-white/15 text-white text-[13px] font-medium hover:bg-white/15 min-w-0 w-full">Cancel</button>
                                <button type="submit" disabled={loading} onClick={handleSubmit as any} className="h-10 rounded-full bg-white text-[#0a0a0b] text-[13px] font-medium hover:bg-neutral-100 disabled:opacity-50 flex items-center justify-center gap-2 min-w-0 w-full">
                        ...
                        <p className="text-[11px] leading-5 text-neutral-500 text-center break-words px-2">By scheduling, you agree to notify patient via WhatsApp.</p>
                    </div>
                </div>
            </form>
        </div>
